...
INSERT INTO CLIENTE 
VALUES ('JOAO', 'M', 'JOAO@GMAIL.COM', 988638273, '22923110', 'MARIA LACERDA - ESTACIO - RIO DE JANEIRO - RJ');
...
INSERT INTO CLIENTE 
VALUES ('ANA', 'F', 'ANA@GLOBO.COM', 85548962, '548556985', 'PRES ANTIONIO CARLOS - CENTRO - SÃO PAULO - SP'),
	   ('CARLA', 'F', 'CARLA@TERATI.COM.BR', 774582, '66587458', 'SAMUEL SILVA - CENTRO - BELO HORIZONTE - MG');
	   
INSERT INTO CLIENTE (NOME, SEXO, ENDERECO, TELEFONE, CPF)
VALUES ('CLARA', 'F', 'SENADOR SOARES - TIJUCA - RIO DE JANEIRO - RJ', '883665843', 11111111111);

/* O COMANDO SELECT */

SELECT NOW() AS DATA_HORA, 'ARTHUR VITAL FONTANA' AS ESTUDANTE ;

SELECT 'ARTHUR VITAL FONTANA';
...
INSERT INTO LIVROS (NOME_LIVRO, NOME_AUTOR, SEXO_AUTOR, NUMERO_PAG, NOME_EDITORA, VALOR, UF_EDITORA, ANO)
	VALUES  ('Cavaleiro Real', 'Ana Caludia', 'F', 465, 'Atlas', 49.9, 'RJ', 2009),
...
SELECT NOME_LIVRO, NUMERO_PAG FROM LIVROS
	WHERE SEXO_AUTOR = 'F';
	
/* 5– Trazer os valores dos livros das editoras de São Paulo. */
SELECT NOME_LIVRO, NOME_EDITORA, VALOR FROM LIVROS 
	WHERE UF_EDITORA = 'SP';
	
/* 6– Trazer os dados dos autores do sexo masculino que tiveram livros publicados por São Paulo ou Rio de Janeiro (Questão Desafio) */
SELECT NOME_LIVRO, NOME_AUTOR, SEXO_AUTOR, UF_EDITORA FROM LIVROS
	WHERE SEXO_AUTOR = 'M'
		AND UF_EDITORA = 'RJ'
			OR SEXO_AUTOR = 'M'
				AND UF_EDITORA = 'SP';
...
SELECT idFuncionario, nome, sexo, admissao, departamento, COUNT(*) AS "Quantidade de funcionarios" FROM FUNCIONARIOS
...
verdadeira, pois a segunda condicao nao sera chacada nesse caso
Resumindo sempre colocar quem tem mais quantidade em primeiro */

SELECT * FROM FUNCIONARIOS 
	WHERE departamento = 'Roupas' 
		OR departamento = 'Filmes';

/* 2- O gestor de marketing pediu a lista das funcionarias que trabalham no departamento de filmes OU no departamento lar.
	Ele necessita enviar u memail para as colaboradoras desses dois setores */

SELECT idFuncionario, nome, sexo, email, departamento FROM FUNCIONARIOS
	WHERE sexo = 'Feminino'
...
/* Como estamos trabalhando com AND, colocamos o departamento primeiro, pois ele irá checar a segunda condicao um numero menor de vezes */

/* 3- Traga os funcionarios do sexo masculino OU os funcionarios que trabalhem no Jardim */
SELECT idFuncionario, nome, sexo, departamento, cargo FROM FUNCIONARIOS 
...
INSERT INTO CLIENTE 
	VALUES ('CARLA', 'F', 'C.LOPEZ@UOL.COM',  45638854, '4575-0048', 'RUA COPPER LEAD - WILLIAMSBURG - CANADA');
	
SELECT * FROM CLIENTE;

DELETE FROM CLIENTE 
	WHERE NOME = 'CARLA'
		AND EMAIL = 'CARLA@YAHOO.COM';
...
QUANTO O CAMPO FOR DIVISIVEL 

3- TODA TABELA NECESSITA DE PELO MENOS UM CAMPO QUE INDENTIFIQUE TODO O REGISTRO 
COMO SENDO UNICO.
...
A OUTRA TABELA PARA FAZER REFERENCIA ENTRE REGISTROS */
 ...
 INSERT INTO CLIENTE
	VALUES(NULL, 'JOAO', 'M', 'JOAOA@IG.COM', '76567587887');
 INSERT INTO CLIENTE
	VALUES(NULL, 'CARLOS', 'M', 'CARLOS@IG.COM', '342342423424');
 INSERT INTO CLIENTE
	VALUES(NULL, 'ANA', 'F', 'ANA@IG.COM', '654577866');
 INSERT INTO CLIENTE
	VALUES(NULL, 'CLARA', 'F', NULL, '11243655542');
 INSERT INTO CLIENTE
	VALUES(NULL, 'JORGE', 'M', 'JORGE@IG.COM', '357675522');
 INSERT INTO CLIENTE
	VALUES(NULL, 'CELIA', 'F', 'CELIA@IG.COM', '23456452354');
...
INSERT INTO ENDERECO 
	VALUES(NULL, 'RUA DO OUVIDOR', 'FLAMNGO', 'RIO DE JANEIRO', 'RJ', 6);
INSERT INTO ENDERECO 
	VALUES(NULL, 'RUA URUGUAIANA', 'CENTRO', 'VITORIA', 'ES', 5);

/* TESTE */	
INSERT INTO ENDERECO 
	VALUES(NULL, 'RUA ALFANDEGA', 'CENTRO', 'SAO PAULO', 'SP', 5);
...
INSERT INTO TELEFONE
	VALUES(NULL,'CEL','87866896',1);
INSERT INTO TELEFONE
	VALUES(NULL,'COM','54768899',2);
INSERT INTO TELEFONE
	VALUES(NULL,'RES','99667587',1);
INSERT INTO TELEFONE
...
A CALUSULA DE SELEÇÃO É O WHERE*/
...
| CELIA  | F    | FLAMNGO | RIO DE JANEIRO |
+--------+------+---------+----------------+
...
SELECT CL.NOME, CL.SEXO, EN.BAIRRO, EN.CIDADE, TL.TIPO, TL.NUMERO
	FROM CLIENTE CL 
		JOIN ENDERECO EN ON CL.IDCLIENTE = EN.ID_CLIENTE
			JOIN TELEFONE TL ON CL.IDCLIENTE = TL.ID_CLIENTE;		
+--------+------+---------+----------------+------+----------+
| NOME   | SEXO | BAIRRO  | CIDADE         | TIPO | NUMERO   |
+--------+------+---------+----------------+------+----------+
...
INSERT INTO CLIENTE VALUES(NULL, 'XXXX', 'M', NULL, 'XXX');
...
/* RELATORIO DE TODOS OS CLIENTES (TELEFONE E ENDERECO)*/

SELECT CL.IDCLIENTE, CL.NOME, CL.SEXO, CL.EMAIL, CL.CPF, EN.RUA, EN.BAIRRO, EN.CIDADE, EN.ESTADO, TL.TIPO, TL.NUMERO
	FROM CLIENTE CL
		JOIN ENDERECO EN ON CL.IDCLIENTE = EN.ID_CLIENTE
			JOIN TELEFONE TL ON CL.IDCLIENTE = TL.ID_CLIENTE;
...
/* QUANTIDADE DE HOMENES E MULHERES */
...
SELECT CL.IDCLIENTE, CL.EMAIL, CL.NOME, CL.SEXO, TL.TIPO, EN.BAIRRO, EN.CIDADE
	FROM CLIENTE CL 
		JOIN ENDERECO EN ON CL.IDCLIENTE = EN.ID_CLIENTE
			JOIN TELEFONE TL ON CL.IDCLIENTE = TL.ID_CLIENTE
				WHERE SEXO = 'F'
					AND EN.BAIRRO = 'CENTRO'
						AND EN.CIDADE = 'Rio de Janeiro'
							AND (TIPO = 'RES' OR 'COM');

/* PARA UMA CAMPANHA DE MARKETING, O SETOR SOLICITOU UM RELATORIO COM O NOME, EMAIL E TELEFONE CELULAR DOS CLIENTES QUE MORAM NO 
	ESTADO DO RIO DE JANEIRO  
	VOCE TERA QUE PASSAR A QUERY PARA GERAR O RELATORIO PARA O PROGRAMADOR */

SELECT CL.NOME, CL.EMAIL, TL.NUMERO, TL.TIPO, EN.ESTADO
	FROM CLIENTE CL --ORIGEM
		JOIN TELEFONE TL ON CL.IDCLIENTE = TL.ID_CLIENTE --JUNÇÃO
			JOIN ENDERECO EN ON CL.IDCLIENTE = EN.ID_CLIENTE --JUNÇÃO
				WHERE EN.ESTADO = 'RJ' --SELEÇÃO
					AND TL. TIPO = 'CEL'; --SELEÇÃO
					
/* PARA UMA CAMPANHA DE PRODUTOS DE BELEZA, O COMERCIAL SOLICITOU UM RELATORIO COM O NOME, EMAIL E TELEFONE CELULAR
	DAS MULHERES QUE MORAM NO ESTADO DE SÃO PAULO
	VOCE TERA QUE PASSAR A QUERRY PARA GERAR O RELATORIO PARA O PROGRAMADOR */
	
SELECT CL.NOME, CL.EMAIL, TL.NUMERO AS CELULAR, TL.TIPO, CL.SEXO, EN.ESTADO
	FROM CLIENTE CL
		JOIN TELEFONE TL ON CL.IDCLIENTE = TL.ID_CLIENTE
			JOIN ENDERECO EN ON CL.IDCLIENTE = EN.ID_CLIENTE
				WHERE CL.SEXO = 'F' 
...
SHOW TABLES; -- AS VIEW FICAM DENTRO DO SHOW TABLES
SHOW DATABASES;
...
VALUES('ANDREIA', 'F', 'ANDREIA@UOL.COM.BR', 'CEL', '873547864', 'CENTRO', 'VITORIA', 'ES'
);
...
/* UPDATE PODE SER FEITO EM VIEWS */
...
/* É POSSIVEL FAZER OPERAÇÕES DML EM VIEW, QUANDO NÃO SE TEM JOIN NELAS */
...
/* ORDER BY POR MAIS DE UMA COLUNA */
...
ORDER BY 1, 2; --NÃO FUNCIONA COM numero
